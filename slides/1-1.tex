\begin{frame}[fragile]{Глава 1: Оптимизация хвостовой рекурсии}
    ...
    возврата из рекурсивного вызова функции \alert{ничего больше делать не нужно}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 1. Начальный вызов}
    \begin{columns}[c]
        ...
        Среда \alert{REPL} вызывает процедуру \mintinline{scheme}{f1} с
        аргументом \mintinline{scheme}{3}
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 3. Вычисление условия}
    \begin{columns}[c]
        ...
        Интерпретатор перешел к вычислению предиката \mintinline{scheme}{(= n 0)}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 4. Условие ложно}
    \begin{columns}[c]
        ...
        Вычисление предиката \mintinline{scheme}{(= n 0)}
        завершилось и вернуло значение \mintinline{scheme}{#f}
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 5. Выбор else-ветки}
    \begin{columns}[c]
        ...
        Форма \mintinline{scheme}{if} получила от
        предиката значение \mintinline{scheme}{#f}.
        Управление передаётся else-ветке c рекурсивным
        вызовом \mintinline{scheme}{(f1 (- n 1))}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 6. Тело обычной (нехвостовой) рекурсии}
    \begin{columns}[c]
        ...
        После выбора else-ветки интерпретатор начал
        вычислять тело процедуры — выражение
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 7. Подготовка рекурсии}
    \begin{columns}[c]
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 8. Вычисление фактического аргумента}
    \begin{columns}[c]
        ...
        В новом рекурсивном вызове \mintinline{scheme}{f1} интерпретатор начал
        вычислять фактический параметр — выражение \mintinline{scheme}{(- n 1)}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 9. Аргумент вычислен}
    \begin{columns}[c]
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 10. Второй уровень рекурсии активирован}
    \begin{columns}[c]
        ...
        Значение аргумента \mintinline{scheme}{2} подставлено:
        теперь выполняется вызов \mintinline{scheme}{(f1 2)}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 11. Стек растёт}
    \begin{columns}[c]
        ...
        Второй вызов \mintinline{scheme}{(f1 2)} — в стеке появился новый фрейм
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 12. Рост стека в нехвостовой рекурсии}
    \begin{columns}[c]
        ...
        Всего за три шага рекурсии (от \mintinline{scheme}{n = 3}
        до \mintinline{scheme}{n = 0})
        стек вырос до восьми активных фреймов.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 14. Первое умножение в сворачивании}
    \begin{columns}[c]
        ...
        Самый глубокий фрейм вернул \mintinline{scheme}{1}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 15. Умножение на самом глубоком уровне завершено}
    \begin{columns}[c]
        ...
        Кадр самого нижнего умножения полностью отработал и
        передал результат \mintinline{scheme}{1} вверх —
        в кадр процедуры \mintinline{scheme}{(f1 1)}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 16. Второй уровень рекурсии завершён}
    \begin{columns}[c]
        ...
        Вызов \mintinline{scheme}{(f1 1)} завершился и
        вернул значение \mintinline{scheme}{1}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 17. Предпоследний шаг сворачивания}
    \begin{columns}[c]
        ...
        Значение \mintinline{scheme}{1} от
        \mintinline{scheme}{(f1 1)} уже
        превратилось во втором уровне в \mintinline{scheme}{2}.
...
\begin{frame}[fragile]{1.1 Обычная рекурсия: Шаг 18. Последнее умножение перед завершением}
    \begin{columns}[c]
        ...
        Умножение второго уровня завершено
        и вернуло \mintinline{scheme}{2}.
        Этот результат подставлен в самое первое умножение:
        \mintinline{scheme}{(* 2 n)} $\to$ \mintinline{scheme}{(* 2 3)}.
